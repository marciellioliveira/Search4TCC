...
public class Conexao {
    ...
    private static Connection conexao; //Cria a variável conexao do tipo Connection
    ...
    private void conectar() {
            ...
            try {
                ...
                conexao = DriverManager.getConnection("jdbc:mysql://localhost:3306/searchfor?user=root&password="); //estabelece a conexao
            } catch (Exception e) {
    ...
    public ResultSet executarConsulta(String sql){ //ResultSet -> Conjunto de resultados
        try {
            conectar(); //Faz a conexão
            Statement stmt = conexao.createStatement(); // Cria a variável do tipo Statement, permitindo a execução de um comando SQL
            ResultSet resultados = stmt.executeQuery(sql); //Cria uma variável do tipo ResultSet que irá guardar os resultados da execução SQL (executeQuery)
            return resultados; //retorna o conjunto de resultados da consulta
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
